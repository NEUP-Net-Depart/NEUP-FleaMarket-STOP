...
class TransactionController extends Controller
{
    ...
    private function buy($good_id, $buyer_id, $count, $request_ip)
    {
        ...
        if($good->seller->wechat_open_id)
            XMSHelper::sendBuyerBoughtMessage($good->seller->wechat_open_id, $trans);
    ...
    private function cancelBuy($trans, $user_id, $request_ip, $reason)
    {
        switch ($user_id) {
            case $trans->buyer_id:
                ...
                if($good->seller->wechat_open_id)
                    XMSHelper::sendBuyerCancelMessage($good->seller->wechat_open_id, $trans);
            ...
            case $trans->seller_id:
                ...
                if($good->buyer->wechat_open_id)
                    XMSHelper::sendSellerRejectMessage($good->buyer->wechat_open_id, $trans);
    ...
    private function sell($trans, $user_id, $request_ip)
    {
        ...
        if($good->buyer->wechat_open_id)
            XMSHelper::sendSellerConfirmMessage($good->buyer->wechat_open_id, $trans);
    ...
    private function finish($trans, $user_id, $result, $request_ip)
    {
        ...
        if($good->buyer->wechat_open_id)
            XMSHelper::sendTransCompleteMessage($good->buyer->wechat_open_id, $trans);
