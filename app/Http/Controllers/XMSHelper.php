...
class XMSHelper extends Controller
{
    static function sendXMSMessage($openid, $templateId, $datas, $link)
    {
        ...
        $key = config('WECHAT_MESSAGE_KEY');

        $xdata["timestamp"] = $timestamp;
        $xdata["data"] = json_encode($data);
        $xdata["bizCode"] = $biz;
    ...
    {
        $data = [];
        array_push($data, [ "name" => "first", "value" => "【先锋市场】" ]);
        array_push($data, [ "name" => "tradeDateTime", "value" => $trans->created_at ]);
        ...
        array_push($data, [ "name" => "orderItemData", "value" => $trans->good->price ]);
    ...
    static function sendSellerConfirmMessage($openid, $trans)
    {
        $data = [];
        array_push($data, [ "name" => "first", "value" => "【先锋市场】" ]);
        array_push($data, [ "name" => "keyword1", "value" => $trans->updated_at ]);
    ...
    static function sendTransCompleteMessage($openid, $trans)
    {
        ...
        array_push($data, [ "name" => "keyword2", "value" => $trans->good->good_name ]);
        array_push($data, [ "name" => "keyword3", "value" => $trans->created_at ]);
        array_push($data, [ "name" => "keyword4", "value" => $trans->updated_at ]);
        array_push($data, [ "name" => "keyword5", "value" => $trans->updated_at ]);
        array_push($data, [ "name" => "orderProductPrice", "value" => $trans->good->price ]);
    ...
    static function sendReplyMessage($openid, $msg)
    {
        ...
        array_push($data, [ "name" => "keyword3", "value" => $msg->created_at ]);
